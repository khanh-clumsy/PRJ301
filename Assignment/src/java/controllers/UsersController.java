    ...
    private void updateUserProfile(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("update") != null) {
            ...
            if ((password == null || password.isEmpty()) && (cpassword == null || cpassword.isEmpty())) {
                password = u.getPassword(); // Nếu không nhập thì giữ nguyên
        ...
        if (request.getParameter("delete") != null) {
            boolean deleted = uda.deleteUsers(id);
            if (deleted) {
                request.getSession().setAttribute("deleteUserMessage", "User deleted successfully!");
                response.sendRedirect("login");
                return;
            } else {
